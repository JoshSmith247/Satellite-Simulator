...
- **Any satellite** — load any NORAD catalog ID at runtime from CelesTrak, with an
  on-disk TLE cache so the app still works offline (cached TLEs are flagged in the
  UI, and the TLE epoch is shown so you can judge freshness).
